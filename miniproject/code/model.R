...
for (i in 1:length(unique(df$ID))){ # loop through unique IDs
  ...
  r_max[i] = coef(fit_logistic)["r_max"]
  K[i] = coef(fit_logistic)["K"]
  N_0[i] = coef(fit_logistic)["N_0"]
  
  logistic_df <- data.frame(AIC_logistic, BIC_logistic, r_max, K, N_0) # Create data frame with AIC and BIC values
}
...
model_stats <- merge(quadratic_df, logistic_df, by = "ID" )
