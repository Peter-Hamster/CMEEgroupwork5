...
rm(list=ls())
test_username <- "keg121" # this should be your username
year <- 2021
...
load_student_work <- function(student_username) {
  ...
  if(file_test("-f",student_file)) {
    ...
    load_test_data[[length(load_test_data)+1]] <-  test_data_updated(name,"name","Kate Griffin")
    load_test_data[[length(load_test_data)+1]] <-  test_data_updated(preferred_name,"preferred_name","Kate")
    load_test_data[[length(load_test_data)+1]] <-  test_data_updated(email,"email","keg121@ic.ac.uk")
